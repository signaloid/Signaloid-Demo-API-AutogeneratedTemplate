...
import { SignaloidWrapperService } from '../../services/signaloid-wrapper.service';
...
import { FilePickerComponent, InputTextComponent } from 'design-system';
...
export class DemoComponent implements OnInit, OnDestroy {
	...
	protected fileUploading: { [inputId: string]: boolean } = {};
	protected fileErrors: { [inputId: string]: string | undefined } = {};
	...
	constructor(
		...
		private signaloidWrapperService: SignaloidWrapperService,
	) {}
	...
	async ngOnInit() {
		...
		this.signaloidWrapperService.getCurrentUser();
	...
	/** Uploads to the user's cloud storage, which the task mounts at sd0. */
	public async onFilePicked(file: File, id: string) {
		const input = demoInputs.find((elem: InputConfig) => elem.id === id);
		if (!input || input.type !== 'file') {
			return;
		}

		const destination = input.destination.replace(/^\/+|\/+$/g, '');
		const storagePath = destination ? `${destination}/${file.name}` : file.name;

		this.fileErrors[id] = undefined;
		this.fileUploading[id] = true;
		try {
			await this.signaloidWrapperService.uploadFile(storagePath, file);
		} catch (error: any) {
			this.fileErrors[id] = makeErrorReadable(error) ?? 'Could not upload the file.';
			return;
		} finally {
			this.fileUploading[id] = false;
		}

		this.state$.next(DemoStates.SECOND_TIME_LOADING);
		this.apiDemoExecutionService.setArgumentsForMultipleDistributions(
			{ value: `sd0/${storagePath}` },
			input.argumentFlag,
		);
		this.executeDemoTask();
	}
