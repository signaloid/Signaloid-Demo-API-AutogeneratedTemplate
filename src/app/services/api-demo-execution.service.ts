...
export class ApiDemoExecutionService {
	...
	public executeDemo() {
		...
		if (this.repository && this.demoObject?.coreId && this.repository?.RepositoryID) {
	...
	private getBuildingObservable(subscriptionId: string, taskId: string) {
		return this.signaloidClient
			.buildRepository(this.repository?.RepositoryID, this.demoObject?.coreId)
			.pipe(
				...
				map((res) => res.data),
				filter(
					(buildResponse: any) => buildResponse.status === 'Completed' || buildResponse.status === 'Stopped'
				),
				mergeMap((buildResponse: any) => {
